...
A single `tools` role installs tool entries from lists in the profiles: apt packages, apt repositories, `.deb` releases, release binaries, release archives, installer scripts and uv tools, plus config files for them. Each entry that is not a plain apt package carries a `verify` command, defaulting to `<name> --version`.
