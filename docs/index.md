...
It is a personal repository, published because the mechanics are reusable: a
role layout that separates generic from OS-specific work, an apt-repository
helper that survives new Ubuntu releases, and a bootstrap script that is safe to
re-run.

## Goals

- **One command on a fresh machine.** `scripts/setup` takes a bare Ubuntu
  install to a working workstation, and is safe to run again afterwards.
- **Every supported Ubuntu LTS.** 22.04, 24.04 and 26.04, with no codename
  hardcoded anywhere.
- **WSL2 first.** The Windows side of the boundary is managed too: Terminal
  settings, fonts, clipboard, `wsl.conf` and `.wslconfig`.
...
See [Getting started](getting-started/index.md) for what that does and what to
set first.
